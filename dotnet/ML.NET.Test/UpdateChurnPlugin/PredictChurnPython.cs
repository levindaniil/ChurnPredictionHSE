...
{
    public class PredictChurnPython : PluginBase
    {
        public PredictChurnPython(Type childClassName) : base(childClassName)
        {
        ...
        {
            var httpWebRequest = (HttpWebRequest)WebRequest.Create("http://localhost:14243/churn/predict");
            httpWebRequest.ContentType = "application/json";
            httpWebRequest.Method = "GET";

            ContactOuput response;
            ...
            using (var streamReader = new StreamReader(httpResponse.GetResponseStream()))
                response = JsonConvert.DeserializeObject<ContactOuput>(streamReader.ReadToEnd());
